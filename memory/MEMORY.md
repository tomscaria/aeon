...
- `memory/topics/papers.md` — **05-12 PhD slot: Heterogeneous Scientific Foundation Model Collaboration / Eywa (`2604.27351`, Zihao Li / Jingrui He et al., UIUC, Apr 30 2026, ↑211 — +93 since queue promotion 05-08) — heterogeneous agentic framework that augments domain-specific predictive foundation models with an LLM reasoning interface; 3 modes (EywaAgent / EywaMAS / EywaOrchestra) across physical/life/social sciences; direct architectural blueprint for stitching CalibrationGap's quant-scanner + LLM-head + regime classifier + narrative scanner under one planner; closes 3-paper architectural arc with AEL (retrieval policy: what to retrieve) and Galanis (aggregation: when to aggregate)**; 05-11 daily slot: Information Aggregation with AI Agents (`2604.20050`, Galanis, econ.GN, 5-claim stack incl. complexity-degrades-aggregation + null-past-performance-feedback); 05-10 daily slot: Signal Credibility Index (`2604.27041`, Nechepurenko, econ.GN cross-list, logit-persistence + Cobb-Douglas concentration + time-varying per-tick spec); 05-09 daily slot: ForesightFlow (`2605.00493`, foundational ILS framework / Murphy decomposition / 24-insider-case inventory / 0.444-magnitude article-vs-automated sign-reversal); 05-08 PhD slot: AEL (`2604.21725`, Sharpe 2.13±0.47); 05-08 daily slot: Per-Market ILS (`2605.02287`). Next-PhD-slot lead: EvoScientist (`2603.08127`, ↑14, multi-agent evolving AI-scientist scaffold — Darwinian-axis complement to Eywa). Next-daily-slot lead: Interest-Bearing Long-Horizon (`2602.21091`, Maresca, Feb 24 2026, econ.GN, ~83% horizon-degradation elimination, Russia-Ukraine EoY-2026 hook). PhD-slot backstop: OracleProto (`2605.03762`, May 5 2026, leakage-controlled reproducible LLM-forecasting benchmark, ≤1% residual leakage).
...
- **Kalshi signals (05-11):** KXFED-27APR T3.50 37%→54% (+17pp, 4pp loose book) — ALERT fired. T3.25 +11pp (thin), T4.00 -14pp (loose, non-monotonic T4.25=42% artifact). Warsh-hawkish slow-cut repricing confirmed at Senate floor vote week. T3.00=53% unchanged (tight 1pp anchor). KXBTC modal shifted B81750→B80750 as BTC retraced from $82k intraday high to $80,722. Ultra-thin dollar volumes — all pattern-read only.
- **Kalshi signals (05-09):** KXBTC-26MAY0817 RESOLVED B80250 — BTC closed $80k-$80.5k, modal bucket called correctly. KXFED-27APR broad repricing: T3.00 46%→53% (+7pp, 1pp spread tight), T4.25 24%→42% (+18pp, 3pp loose, non-monotonic artifact). Ultra-thin vol ($60-$70/market/day) — pattern read only, not actionable. Consistent with Warsh-hawkish slower-cut trajectory.
...
- **swarm-fund-mvp 72h merge-cadence test** — does new ADR open by 2026-05-09, or do PRs #30/#31 stall? Decides whether this week is healthy defect-hardening (this week's article) or queue stagnation (yesterday's article framing). _(NEEDS ATTENTION: deadline today — verify swarm-fund-mvp for new ADR)_
...
- **`paper-pick` next slots** → PhD: **EvoScientist (`2603.08127`, ↑14, Mar 2026)** — promoted 05-12 after Eywa (`2604.27351`) picked same day PhD slot; multi-agent evolving AI-scientist scaffold; pairs as Darwinian-axis complement to Eywa's heterogeneous-FM-coordination axis. Backstop: **OracleProto (`2605.03762`, May 5 2026, Yiding Ma et al., ↑1)** — leakage-controlled reproducible LLM-forecasting benchmark (model-cutoff-aligned sample admission + tool-level temporal masking + content-level leakage detection + discrete answer normalization + hierarchical scoring), reduces residual leakage to ≤1% (order of magnitude tighter than tool-only filtering); demoted from lead because already exposed via 05-07 paper-digest one-of-five. Daily: **Interest-Bearing Long-Horizon (`2602.21091`, Caleb Maresca, Feb 24 2026, econ.GN)** — agent-based LLM-trader sim, interest-bearing positions eliminate ~83% of horizon-related accuracy degradation and lift participation 17%→62%; direct hook for the Russia-Ukraine EoY-2026 long-horizon market; would extend bench to 11-paper economics-and-trading. Daily backstop: Arbitrage Analysis in Polymarket NBA Markets (`2605.00864`, Cheng/Yang/Zou, Apr 22 2026), or first non-queued mission-axis paper from HF browse.
- **Cite stack for next grant / Stanford application** — Cong (`2604.20421`, Stanford anchor) + PolySwarm + Anatomy + Foresight Arena + **ForesightFlow (`2605.00493`, foundational ILS framework)** + ILS-dl Iran-cluster (`2605.02286`, empirical anchor) + Per-Market ILS (`2605.02287`, methodology formalization) + **SCI (`2604.27041`, price-move-credibility axis, econ.GN cross-list, two failure modes pre-specified)** + **Galanis Information Aggregation (`2604.20050`, econ.GN primary, controlled experiment, complexity-threshold finding, null past-performance feedback)** + Coordination Layer + TimeSeek + Prediction Arena (head-to-head: grok-4-20 71.4% PM vs CalibrationGap 76%/29) + AEL (`2604.21725`, Sharpe 2.13) + LBS-Yale 1.72M-account/3.14%-informed + Deflated Sharpe Ratio. **3-paper Nechepurenko ILS arc + SCI + Galanis** = 5-paper microstructure-credibility-aggregation stack; 2-paper-in-48h q-fin.TR→econ.GN voice tilt (SCI 05-10 + Galanis 05-11). 10-paper economics-and-trading bench against which CalibrationGap's 76%/29-trade record can falsify-or-confirm.
