...
- [COMBINED §0.5 review disposition](../../swarm-fund-mvp/docs/superpowers/specs/2026-06-05-COMBINED-architecture-and-data-plan.md) — accept/critique/defer per reviewer; defended plane-split + 11-name EventKind alphabet against "kill the Source Protocol" / "cut to 3 kinds"; flagged that Aristotle's "10,000 Revenants" GC concern is **already solved** in ADR-119 §5 decay pass (he reviewed specs, not the ADR); operator-attention pricing RESOLVED $0 / deferred. Commit `90ac169c`.
- **ADRs 121-128 + 3 amendments** appended to DECISIONS.md (commit `c9639632`) — ADR-121 (Source Protocol target), ADR-122 (DuckDB cache-not-tier), ADR-123 (ClickHouse Week 1 + cost re-split), ADR-124 (Container = manifest+book, not agent), ADR-125 (IC+DSR two-tier ladder; Container C == WT-I), **ADR-126 KILLED** (Path B LoRA folded into ADR-106 per R4), ADR-127 (crosscutting Source contract — secrets/obs/deploy), ADR-128 (Data-ROI five verdicts with intervention-gated CUT). Amend ADR-118 (B1 alphabet canonical), ADR-119 (B3 lifecycle mapping; demotion must keep return stream flowing), ADR-111 (Platt adoption checklist).
- **Parallel dispatch** via `superpowers:dispatching-parallel-agents` skill — 9 agents fired:
  - 5 writing-plans landed at `docs/superpowers/plans/`: WT-A canonical event ontology, WT-B source protocol minimal, WT-F warehousing W4, W3 Tier-1 sources, W5 BacktestEngine.
  - 4 impl agents (worktree-isolated): Platt adoption ✓ merged, Phase-0 Data-ROI plumbing ✓ merged, signal_log_rejected (running), corpus canonicalization (running).
- **Week-0 ship-now lane LIVE on main:**
  - **Phase-0 Data-ROI plumbing** (`d73a84b6`): `CostEvent.data_source` field + `config/billing_registry.yml` + `scripts/data_roi_alarm.py` + 23 tests. Smoke fires the **4 NOT_YET_PLUMBED lines** (SF / Dune / predictiondata / Adjacent — every paid source has zero consumers, exactly as Data-ROI spec §9 predicted). This is the founder R5 ask operationalized.
  - **W8 Platt adoption** (`3e30ade0`): wired at `paper_triage` (3 LLM sub-scores) + `calibration_gap.confidence`; default α=√3 per AIA Forecaster; autoresearch rightly skipped (no LLM probability in the ranking path, documented at `docs/02_architecture/platt_adoption_gaps.md`). 16 new tests, 45/45 green.
- **Adjacent CS audit runner** ready for the founder at `outputs/2026-06-05_adjacent_audit_runner.py` — self-contained read-only probe, writes verdict under B5 default-KEEP semantics.
- **Manual file refreshed** at `outputs/manual_tasks_thomas.md` with the 2026-06-05 founder asks (SF HOBBY signup, Dune Plus, Yanni pickup, Adjacent audit).
